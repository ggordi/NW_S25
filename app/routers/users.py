...
def post_user_trade(id: int, trade: dict = Body(...)):
  ...
  if not keys.issubset(trade.keys()):
    raise HTTPException(status_code=400, detail="Insufficient trade data provided")
  ...
  if not isinstance(trade["share_amount"], int) or trade["share_amount"] <= 0:
    raise HTTPException(status_code=400, detail="Invalid share amount provided")

  # get the user's portfolio
  response = client.table("portfolio").select("*").eq("user_id", id).execute()
  assets = response.data or []  # assets is a list of dictionaries, each dict representing an asset 
    ...
    if any(asset["ticker"] == trade["ticker"] for asset in assets):
      for asset in assets:
        if asset["ticker"] == trade["ticker"]:
          ...
          client.table("portfolio").update({"share_amount": asset["share_amount"]}).eq("id", asset["id"]).execute()
          return {"message": f"Bought {trade['share_amount']} shares of {trade['ticker']}"}
    else:
      # if the user does not have the asset in their portfolio, add a new row
      client.table("portfolio").insert({"user_id": id, "ticker": trade["ticker"], "share_amount": trade["share_amount"]}).execute()
      return {"message": f"Bought {trade['share_amount']} shares of {trade['ticker']}"}
  ...
  # if the user wants to sell an asset...
    ...
    if not any(asset["ticker"] == trade["ticker"] for asset in assets):
      raise HTTPException(status_code=400, detail="Cannot sell asset that does not exist in portfolio")
    else:
      ...
      for asset in assets:
        if asset["ticker"] == trade["ticker"]:
          if asset["share_amount"] < trade["share_amount"]:
            ...
            asset["share_amount"] -= trade["share_amount"]
            client.table("portfolio").update({"share_amount": asset["share_amount"]}).eq("id", asset["id"]).execute()
            return {"message": f"Sold {trade['share_amount']} shares of {trade['ticker']}"}

  # TO FIX: pull pricing data from yf and adjust entry to average purchase price 
